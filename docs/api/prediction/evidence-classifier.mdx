...
description: "EvidencePredictor fixed to classification: predict returns labels and predict_proba returns probabilities in classes_ order."
---


```python
from mekiki import EvidenceClassifier
```
...
[`EvidencePredictor`](/api/prediction/evidence-predictor) fixed to classification: `predict` returns labels and
`predict_proba` returns probabilities in `classes_` order.

Takes every argument of [`EvidencePredictor`](/api/prediction/evidence-predictor) except `task` and `unit`. The labels are
